...
from sentence_transformers import SentenceTransformer #this is added to embedd the query and the document to do MMR
    ...
    def inference(self, question, demo, case):
        ...
        while True:
            ...
            if not check_info.hallucination:
                ...
                #----------------------MMR PART---------------------------------------------------------------------------------------
                raw_docs = self.retriever(retrieve_qry, topk=20) #we'll start by taking the top 20 documents more relevant to the query 
                model_MMR = SentenceTransformer("all-MiniLM-L6-v2") #the model used to generate the embeddings
                doc_embeddings = model_MMR.encode(raw_docs, convert_to_numpy=True) #encode the documents chosen 
                query_embedding = model_MMR.encode([retrieve_qry], convert_to_numpy=True) #encode the query
                def mmr(doc_embeddings, query_embedding, lambda_param=0.7, top_k=5): 
                    selected = []
                    candidates = list(range(len(doc_embeddings)))
                    sim_to_query = cosine_similarity(doc_embeddings, query_embedding).reshape(-1)
                    ...
                    for _ in range(top_k):
                        ...
                        for i in candidates:
                            if selected:
                                redundancy = max(sim_between_docs[i][j] for j in selected)
                            else:
                                redundancy = 0
                            score = lambda_param * sim_to_query[i] - (1 - lambda_param) * redundancy
                            if score > best_score:
                                best_score = score
                                best_idx = i
                    ...
                    return selected
                selected_indices = mmr(doc_embeddings, query_embedding, lambda_param=0.7, top_k=self.retrieve_topk)
                docs = [raw_docs[i] for i in selected_indices]
                #-----------------------------------MMR PART-----------------------------------------------------------------------------
